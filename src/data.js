...
const randomizeQuestions = (data) => {
    const tenQs = []
    let alreadyUsed = {}
    for(let i = 0; i <10; i++){
        let randomIdx = Math.floor(Math.random() * (data.length - 1))
        if(!alreadyUsed[randomIdx]){
            tenQs.push(data[randomIdx])
            alreadyUsed[randomIdx] = true
        }
        else {
          i--
        }
...
module.exports = {randomizeQuestions, allQuestions}
